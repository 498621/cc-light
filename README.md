...
除了实时状态，它还内置**历史会话查看器**、**用量统计**和 **Claude Code 配置编辑器**：回看每个会话的对话 / token / 耗时 / 改动文件，跨会话汇总用量，以及带校验地可视化编辑 CLAUDE.md 与 settings.json。
...
菜单栏   🔴2 🟢1 ⚪3          ← 分状态计数模式（或合并为一个灯：🔴）
   ...
   │ 历史会话管理…                     │   ← 会话/任务统计、对话回看
   │ 用量统计…                         │   ← 跨会话 token / 成本汇总
   │ Claude Code 配置编辑…             │   ← 编辑 CLAUDE.md / settings.json
   │ ──────────────────────────────── │
   │ ☐ 菜单栏分状态计数                │
   │ ☑ 开机自启动                     │
   │ ──────────────────────────────── │
...
- **历史会话查看器** —— 单击任务即在右栏看详情（耗时 / token / 改动文件 / 提交 / Claude 完整对话）；全文搜索命中高亮、会话收藏星标、一键复制 `claude --resume`。
- **用量统计** —— 跨所有会话按模型 / 项目 / 日期汇总 token 与成本，含近 30 天每日柱状时间轴。
- **配置编辑器** —— 直接编辑 `~/.claude/CLAUDE.md` 与 `settings.json`：JSON 高亮 + 保存校验 + 自动美化，并留存每次修改的 diff 历史。
...
| 历史 / 统计 / 配置 | 点圆点 →「历史会话管理…」「用量统计…」「Claude Code 配置编辑…」 |
...
下拉菜单里的三个入口，各打开一个原生窗口：

- **历史会话管理…** —— 左侧会话列表（全文搜索 prompt / 返回内容，命中处高亮；行尾星标可收藏），中间是选中会话的概览（任务数 / 耗时 / token / 成本，右上角一键复制 `claude --resume <id>`），右侧**单击任务**即显示详情：耗时 / token / 红灯、改动的文件（点击复制路径）、你的提交、Claude 的完整 markdown 回复，上/下一条翻看。
- **用量统计…** —— 跨所有会话按模型 / 项目 / 日期汇总会话数、任务数、token 与成本，含近 30 天每日用量柱状时间轴（悬停看明细）。
- **Claude Code 配置编辑…** —— 编辑 `~/.claude/CLAUDE.md`（纯文本）与 `settings.json`（JSON 语法高亮 + 保存时校验，非法则提示不写、合法则美化回写）；右上「修改历史」查看本工具历次改动的 diff。

> 历史与用量的数据全部来自 Claude Code 自己记录的会话文件（`~/.claude/projects/**/*.jsonl`），只读解析、不改动。成本按内置价目表估算，可在 `stats.py` 的 `MODEL_PRICING` 里按官方价调整。配置编辑会真实写入对应文件，每次保存的 diff 存在本地供回看。
...
- macOS
- Python 3.9+，依赖 `rumps`、`pyobjc-framework-WebKit`、`markdown`（`start.sh` 自动安装；后两个供历史 / 用量 / 配置窗口渲染用）
...
├── history_window.py     # 历史会话窗口（AppKit）：列表 / 搜索 / 收藏 / 概览 / 任务 / 详情
├── usage_window.py       # 用量统计窗口（WKWebView dashboard + 每日时间轴）
├── config_window.py      # 配置编辑窗口：CLAUDE.md / settings.json + 修改历史 diff
